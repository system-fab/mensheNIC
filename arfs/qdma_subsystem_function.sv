...
/*qdma_subsystem_hash hash_inst (
  ...
  always @(posedge axis_aclk) begin
      if (~axil_aresetn) begin
          ...
          cam_in <= axis_c2h_tdata[116 +: 12];
          if ((axis_c2h_tdata[320 +: 16] == 16'hf2f2) && first_frag) begin   // "first_frag" is used to check that this is the first segment of the packet (where are the headers), 
              ...
              buffered_axis_c2h_tvalid <= 0;
              queue_conf <= axis_c2h_tdata[368 +: 11];
          end
          else begin
